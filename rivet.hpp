// rivet: https://github.com/onihusube/rivet/tree/e4647083e3cb891bdc3f82fb7ad398f4f770cab4/include
...
namespace rivet::detail
{
	...
	struct range_adaptor_closure_base : public detect_special_ambiguous_case
	{
		...
		requires requires(R&& in, const Adaptor& raco) {
					 {
						 raco(std::forward<R>(in))
						 } -> std::ranges::view;
				 }
		[[nodiscard]] friend constexpr auto operator|(R&& r, const Adaptor& self) noexcept(std::is_nothrow_invocable_v<const Adaptor&, R>)
		{
		...
		[[nodiscard]] friend constexpr auto operator|(LHS lhs, Adaptor self) noexcept(std::is_nothrow_move_constructible_v<LHS>and std::is_nothrow_move_constructible_v<Adaptor>)
		{
		...
		[[nodiscard]] friend constexpr auto operator|(Adaptor self, RHS rhs) noexcept(std::is_nothrow_move_constructible_v<Adaptor>and std::is_nothrow_move_constructible_v<RHS>)
		{
...
namespace rivet
{
	...
	struct closure : F
		, detail::dispatcher<closure<F>>::type
	{
		// ここは右辺値受けのムーブで十分
		// bind_back()にせよラムダにせよ、右辺値で渡ってくる用法しか考慮しない
		constexpr closure(F&& f) noexcept(std::is_nothrow_move_constructible_v<F>)
			: F(std::move(f)) {}
	};
} // namespace rivet
...
namespace rivet::detail
{
...
#endif
	{
		...
		constexpr auto operator()(Args&&... args) const noexcept((std::is_nothrow_constructible_v<Args, Args> && ...) and (std::is_nothrow_move_constructible_v<Args> && ...))
		{
...
namespace rivet
{
	...
	public:
		...
		constexpr adaptor(F&& f) noexcept(std::is_nothrow_move_constructible_v<F>)
			: m_f(std::move(f)) {}
